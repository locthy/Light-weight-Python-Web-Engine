...
def resolve_routing_policy(hostname, routes):
    ...
    #print(hostname)
    proxy_map, policy = routes.get(hostname,('172.18.0.1:9000','round-robin'))
    ...
    if isinstance(proxy_map, list):
        ...
        #   policy
            ...
            if (policy or "").lower() == "round-robin":
                with _rr_lock:
                    i = _rr_idx.get(hostname, 0) % len(proxy_map)
                    _rr_idx[hostname] = i + 1
                target = proxy_map[i]
            else:
                target = proxy_map[0]
...
def run_proxy(ip, port, routes):
    ...
    try:
        ...
        while True:
            ...
            #
            client = threading.Thread(target=handle_client, args=(ip, port, conn, addr, routes))
            client.daemon = True
            client.start()
    except socket.error as e:
      print("Socket error: {}".format(e))
